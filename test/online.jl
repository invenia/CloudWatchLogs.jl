@testset "Online" begin
    ...
    CFG = global_aws_config(;
        creds=assume_role(CI_USER_CFG, TEST_ROLE, Dict("DurationSeconds" => "7200"))
    )
    LOG_RUN_ID = uuid1()
    ...
    @testset "Create/delete groups and streams" begin
        @testset "Named group" begin
            ...
            @test create_group(CFG, group_name; tags=Dict("Temporary" => "true")) ==
                  group_name
        ...
        @testset "Not allowed" begin
            ...
            @test_throws AWSException delete_stream(
                CFG, FORBIDDEN_LOG_GROUP, FORBIDDEN_GROUP_LOG_STREAM
            )
        end
    end
    ...
    @testset "CloudWatchLogStream" begin
        ...
        @testset "Not allowed" begin
            @test_throws AWSException CloudWatchLogStream(
                CFG, FORBIDDEN_LOG_GROUP, FORBIDDEN_GROUP_LOG_STREAM
            )

            stream = CloudWatchLogStream(
                CFG, BAD_STREAM_LOG_GROUP, FORBIDDEN_LOG_STREAM, nothing
            )
        ...
        @testset "Unsorted logs" begin
            ...
            stream = CloudWatchLogStream(
                CFG, TEST_LOG_GROUP, create_stream(CFG, TEST_LOG_GROUP, stream_name)
            )
        ...
        @testset "Stream not found" begin
            @test_throws LOGGER StreamNotFoundException CloudWatchLogStream(
                CFG, TEST_LOG_GROUP, "made-up-stream-that-doesnt-exist"
            )
        end
    end
